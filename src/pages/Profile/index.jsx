...
const Profile = () => {
  ...
  const handleEditProfile = async (formData) => {
    try {
      ...
      const response = await axios.put("http://localhost:9000/users/updateProfile", formData);
  ...
  const updateProfile = useCallback(async () => {
    try {
      const response = await axios.get(`http://localhost:9000/users/${user._id}`);
      ...
      <div className="mt-32 mx-10">
        ...
        <div className="mx-auto flex justify-center text-xl divide-x divide-gray-500 mt-10 text-primary border-b pb-4 border-gray-400 w-[30rem]">
          ...
          <Link className="px-12 flex gap-2" to={"following"}>
            <span className="text-xl italic font-bold">{user?.following.length}</span>{" "}
            Following
          </Link>
          <Link className="px-12 flex gap-2" to={"followers"}>
            <span className="text-xl italic font-bold">{user?.followers.length}</span>{" "}
            Followers
          </Link>
        </div>
        <Outlet />
