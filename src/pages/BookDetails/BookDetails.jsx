...
import { FaHeart, FaRegHeart, FaStar, FaDownload, FaBookReader, FaTrash } from "react-icons/fa";
...
const BookDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      try {
        const response = await axios.get(`http://localhost:9000/ratings/${id}/reviews/first-five`);
  ...
  const toggleFavorite = async () => {
    ...
    try {
      let response;
      if (isFavorite) {
        response = await axios.post(`http://localhost:9000/books/removeFavoriteBook/${userId}/${id}`);
      } else {
        response = await axios.post(`http://localhost:9000/books/addFavoriteBook/${userId}/${id}`);
      }
  ...
  const handleAddReview = async (e) => {
    ...
    try {
      const response = await axios.post(`http://localhost:9000/ratings/${userId}/${id}`, {
        rating,
        review: reviewText,
      });
  ...
  const handleSeeAllReviews = async () => {
    try {
      const response = await axios.get(`http://localhost:9000/ratings/${id}/reviews/all`);
      setReviews(response.data.reviews);
        ...
        <div className="book-content bg-gray-50 shadow-md px-6 rounded-md my-10 h-auto">
          <div className="flex flex-row justify-end items-end mb-3 px-6 pt-5">
            {isFavorite ? (
              ...
              <FaRegHeart className="text-2xl cursor-pointer text-red-500" onClick={toggleFavorite} />
            )}
          </div>
          ...
          <div className="reviews">
            {reviews.map((review, index) => (
              (!showAllReviews && index < 5) || showAllReviews ? (
                <div className="flex items-start mb-4 p-4 border-b" key={review._id}>
                  <div className="mr-4 flex-shrink-0">
                    {review.user.photo ? (
                      <img src={`http://localhost:9000${review.user.photo}`} alt={review.user.name} className="w-10 h-10 rounded-full object-cover" />
                    ) : (
            ...
            {!showAllReviews && reviews.length > 5 && (
              <button onClick={handleSeeAllReviews} className="text-green-500 mt-4">
                See All Reviews
              </button>
            )}
          </div>
